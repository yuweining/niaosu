 ...
 * @license GNU General Public License 2.0
 */
class Mirages_Widget_Metas_Category_List extends Widget_Abstract_Metas
{
    /**
     * 树状分类结构 
     * 
    ...
    private function treeViewCategoriesCallback()
    {
        ...
        echo '"><a href="' . $this->permalink . '">' .  _mt($this->name) . '</a>';
    ...
    public function treeViewCategories() {
        ...
        echo '<a class="nav-link dropdown-toggle" href="#" id="dropdown-menu-' . $this->slug . '" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
        echo _mt($this->name);
        echo '</a>';
        ...
        if ($children) {
            ...
            echo '<' . $this->_categoryOptions->wrapTag . (empty($this->_categoryOptions->wrapClass)
                ? '' : ' class="' . $this->_categoryOptions->wrapClass . '"') . ' aria-labelledby="dropdown-menu-' . $this->slug . '">';

            echo '<' . $categoryOptions->itemTag . ' class="' . implode(' ', $classes) . ' category-parent"><a href="' . $this->permalink . '">' . _mt('分类下所有') . '</a>' . '</' . $categoryOptions->itemTag . '>';
    ...
    public function listCategories($categoryOptions = NULL)
    {
        ...
        Mirages::$options->lastTopCategorySize = count($this->_top);
        if ($this->have()) {
            while ($this->next()) {
                if ($this->children) {

//                    echo '<' . $this->_categoryOptions->wrapTag . ' class="' . $this->_categoryOptions->wrapClass . '"' . '>';
                    $this->treeViewCategories();
//                    echo '</' . $this->_categoryOptions->wrapTag . '>';

                } else {
                    echo '<li class="nav-item"><a class="nav-link" href="' . $this->permalink . '">' . _mt($this->name) . '</a></li>';
                }
            }
        }

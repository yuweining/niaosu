...
class Device {
    ...
    public static function isSierraOrAbove(){
        $version = self::getMacOSXVersion();
        if ($version && $version >= 1012) {
            return true;
        }
        return false;
    }
    public static function isBigSurOrAbove(){
        $version = self::getMacOSXVersion();
        if ($version && $version >= 1100) {
            return true;
        }
        return false;
    }
    public static function getMacOSXVersion(){
        ...
        if (preg_match('/^.+Mac\s+OS\s+X\s+(\d+)[^a-zA-Z0-9]+(\d+).*$/i', self::getUserAgent(), $matches)) {
            ...
            return intval($mainVersion) * 100 + intval($subVersion);
        }
        return false;
    }

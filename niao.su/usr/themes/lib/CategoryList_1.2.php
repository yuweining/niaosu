 ...
 */
class Mirages_Widget_Metas_Category_List extends Metas
{
    /**
     * 树状分类结构
     *
    ...
    public function listCategories($categoryOptions = null)
    {
        ...
        Mirages::$options->lastTopCategorySize = count($this->top);
        if ($this->have()) {
            while ($this->next()) {
                if ($this->children) {
    ...
    public function treeViewCategories()
    {
        ...
        if ($children) {
            ...
            echo '<' . $this->categoryOptions->wrapTag . (empty($this->categoryOptions->wrapClass)
                    ? '' : ' class="' . $this->categoryOptions->wrapClass . '"') . ' aria-labelledby="dropdown-menu-' . $this->slug . '">';
